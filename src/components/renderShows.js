...
export const renderShows = async (
  ...
  try {
    ...
    if (filterActionMovies) {
      ...
      const filteredComedyShows = shows.filter((show) =>
        show.genres.includes("Comedy")
      );
      pagination.removeEventListener("click", handlePageClick); // Remove previous event listener
      pagination.addEventListener("click", (event) =>
        handlePageClick(event, filteredComedyShows)
      );
      ...
      const filteredCrimeShows = shows.filter((show) =>
        show.genres.includes("Crime")
      );
      pagination.removeEventListener("click", handlePageClick); // Remove previous event listener
      pagination.addEventListener("click", (event) =>
        handlePageClick(event, filteredCrimeShows)
      );
